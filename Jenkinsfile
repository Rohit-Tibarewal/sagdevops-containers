        ...
        stage('Run') {
            steps {
                script {
                  dir ('./containers') {
                        docker.withRegistry("https://${params.sourceContainerRegistryHost}", "${params.sourceContainerRegistryCredentials}"){
                            sh "docker-compose --env-file /var/lib/jenkins/workspace/msr-pipeline/containers/microservices-runtime/config/env.dev up -d --force-recreate --remove-orphans ${params.buildScenario}"
                        }
                    }
                }
            }
